      ...
      <div className="relative h-16 overflow-hidden">
        ...
        {chapters > 1 && (
          <div className="absolute top-2 right-3">
            <span className="text-[8px] font-bold tracking-wider" style={{ color: 'rgba(200,210,225,0.65)' }}>
...
export default function DiscoveriesPage() {
  ...
  const [movingOn, setMovingOn] = useState<{ entry: VaultEntry; creator: Creator } | null>(null)
  ...
  const activityEntries = [...visibleEntries, ...archivedEntries]
    .sort((a, b) => b.spotDate.getTime() - a.spotDate.getTime())
            ...
            {activityEntries.length === 0 ? (
              ...
              activityEntries.map((entry, i) => {
                ...
                return (
                  ...
                  >
                    <div className={cn('w-2 h-2 rounded-full flex-shrink-0', isArchived ? 'bg-hype-muted' : 'bg-hype-green')} />
                    <div className="flex-1 min-w-0">
                      <p className="text-hype-text text-sm font-semibold">
                        {isArchived ? 'Moved On' : 'Spotted'}{' '}
                        <span className="font-mono text-hype-secondary">${entry.ticker}</span>
                      </p>
                      {creator && <p className="text-hype-secondary text-xs">{creator.name}</p>}
                      <p className="text-hype-muted text-xs mt-0.5">{formatTimeAgo(entry.spotDate.toISOString())}</p>
                    </div>
                    <div className="text-right">
                      <p className={cn('text-xs font-semibold uppercase tracking-wide', isArchived ? 'text-hype-muted' : 'text-hype-green')}>
                        {isArchived ? '− Moved On' : '+ Spotted'}
                      </p>
                    </div>
                  </motion.div>
